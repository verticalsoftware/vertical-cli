...
- Step 1: Define object types that the application's user arguments will be mapped to. These objects can be expressed as classes, records, or interfaces (when using the source generator).
- Step 2: Configure the parser by specifying the positional arguments, options, and switches the application supports.
...
The primary purpose of the framework is to convert string arguments into strongly typed objects. Therefore, each command in the application that performs a function has a options model that is defined by the application. Models can be written as:
- Records
- Classes with settable properties
- Interfaces

The model contains the properties that the parser will bind arguments values to. The handler of the command receives a composed model object and performs the application's function.
...
|Classes|Arguments are mapped to properties with a `set` or `init` accessor. Classes either fit the `new()` type constraint or the application can provide an activator.|
...
- The following collection types provided the underlying type is any one of the above:
    ...
    - Collection interfaces implemented by any of the above collection types. The source generator will use an appropriate concrete implementation during instantiation.
...
- A _positional argument_ is a value without an identifier. The parser recgonizes positional arguments when it cannot match an option or switch identifier.
...
A posix group is a hyphen followed by a character set. This convention can only be used by the application user. The parser expands the group token into separate tokens for matching, e.g. `-abc` is expanded to `-a`, `-b`, and `-c`. In this specific scenario, the first two expanded tokens are matched to switches, while the last token is matched to an option or a switch. If the last token is an option, then it can have an attached parameter value or a trailing parameter value argument, e.g. `-abc:value` is expanded to `-a`, `-b`, and `-c:value`.
