...
public sealed class HelpTextWriter
{
    ...
    public void WriteSection(string heading, string content, (int Left, int Right) margin)
    {
        Write(heading);
        WriteLine();
        WriteBlock(content, margin);
    }

    public void WriteTerm(string term, string content, (int Left, int Right) margin)
    {
        Guard.IsNotNull(term);
    ...
    public void WriteBlock(string content, (int Left, int Right) margin)
    {
        Guard.IsNotNull(content);

        WriteBlock(content.AsSpan(), margin);
    }

    public void WriteBlock(ReadOnlySpan<char> span, (int Left, int Right) margin)
    {
        if (span.Length == 0)
            return;
    ...
    public void WriteToMargin(int margin) => WriteNewLineLeftMargin(margin);
